...
from solveLOPGraphical import Constraint, LinearOptimizationProblem, total_intersections
...
def create_random_lop_problem():
    ...
    return LinearOptimizationProblem(constraints = constraints, objective = random_objective())

def prettify(text: str):
    return text.replace(". ", ".\n")

def create_random_problem():
    random_lop_problem = create_random_lop_problem()
    client = OpenAI()
    completion = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "developer", "content": "Du bist ein Lehrer und erstellstst eine Lineare Optimierungsaufgabe."},
    ...
    print("\n" * 8)
    print("=" * 30)
    print(random_lop_problem)

if __name__ == '__main__':
    print(create_random_problem())
